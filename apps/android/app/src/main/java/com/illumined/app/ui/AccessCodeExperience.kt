...
fun AccessCodeExperience(profile: UserProfile, parishMode: Boolean, onBack: () -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize().background(codeBrush()), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        if (!parishMode && classId.isNotBlank()) item {
            CodeCard {
                Text("Student Class Link", fontSize = 18.sp, fontWeight = FontWeight.SemiBold, color = IlluminedThemeTokens.Blue)
                Text("Share this reusable link with students joining class $classId.", color = IlluminedThemeTokens.SecondaryText)
                InviteShareControls(IlluminedInviteLink(InviteRole.STUDENT, classId = classId))
            }
        }
